		...
		<div class="row">
			...
			<div class="col-md-6" style="float:left;">
				<div class="card">
					<div class="card-header">
						<b>Expired Product List</b>
			<button class="col-md-2 float-right btn btn-primary btn-sm" id="new_expired2"><i class="fa fa-plus"></i> New Entry</button>
					</div>
					<div class="card-body">
						<table id="table2" class="table table-bordered">
							...
							<?php 
							$i = 1;
								$expired = $conn->query("SELECT e.*,p.name,p.sku,p.measurement FROM expired_product2 e inner join product_list p on p.id = e.product_id order by date(e.date_created) desc");
								while($row=$expired->fetch_assoc()):
							?>
								<tr>
									<td class="text-center"><?php echo $i++ ?></td>
									...
									<td class=""><?php echo $row['qty'] ?></td>
									<td class="text-center">
										<a class="btn btn-sm btn-danger delete_expired2" href="javascript:void(0)" data-id="<?php echo $row['id'] ?>">Delete</a>
									</td>
								</tr>
							<?php endwhile; ?>
							</tbody>
...
<script>
	...
	$('#table2').dataTable()
	$('#new_expired2').click(function(){
		location.href = "index.php?page=manage_expired2"
	})
	$('.delete_expired2').click(function(){
		_conf("Are you sure to delete this data?","delete_expired2",[$(this).attr('data-id')])
	})
	function delete_expired2($id){
		start_load()
		$.ajax({
			url:'ajax.php?action=delete_expired2',
			method:'POST',
			data:{id:$id},
			success:function(resp){
				if(resp==1){
