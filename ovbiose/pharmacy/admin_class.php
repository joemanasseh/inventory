...
Class Action {
	...
	function save_product2(){
		...
		if(empty($sku)){
			...
			$i = 1;
			while($i == 1){
				$chk = $this->db->query("SELECT * FROM product_list2 where sku ='$sku'")->num_rows;
				if($chk > 0){
					$sku = mt_rand(1,99999999);
					$sku = sprintf("%'08d\n", $sku);
				}else{
		...
		if(empty($id)){
			$save = $this->db->query("INSERT INTO product_list2 set ".$data);
		}else{
			$save = $this->db->query("UPDATE product_list2 set ".$data." where id=".$id);
		}
		if($save)
			return 1;
	}
	...
	function delete_product2(){
		extract($_POST);
		$delete = $this->db->query("DELETE FROM product_list2 where id = ".$id);
		if($delete)
			return 1;
	}
	...
	function save_receiving2(){
		...
		if(empty($id)){
			...
			while($i == 1){
				$chk = $this->db->query("SELECT * FROM receiving_list2 where ref_no ='$ref_no'")->num_rows;
				if($chk > 0){
					$ref_no = mt_rand(1,99999999);
					$ref_no = sprintf("%'.08d\n", $ref_no);
				}else{
			...
			$data .= ", ref_no = '$ref_no' ";
			$save = $this->db->query("INSERT INTO receiving_list2 set ".$data);
			$id =$this->db->insert_id;
			foreach($product_id as $k => $v){
				...
				$save2[]= $this->db->query("INSERT INTO inventory2 set ".$data);
			}
			if(isset($save2)){
				return 1;
			}
		}else{
			$save = $this->db->query("UPDATE receiving_list2 set ".$data." where id =".$id);
			$ids = implode(",",$inv_id);
			$this->db->query("DELETE FROM inventory2 where type = 1 and form_id ='$id' and id NOT IN (".$ids.") ");
			foreach($product_id as $k => $v){
				$data = " form_id = '$id' ";
				$data .= ", product_id = '$product_id[$k]' ";
				$data .= ", qty = '$qty[$k]' ";
				...
				$data .= ", remarks = 'Stock from Receiving-".$ref_no."' ";
				if(!empty($inv_id[$k])){
									$save2[]= $this->db->query("UPDATE inventory2 set ".$data." where id=".$inv_id[$k]);
				}else{
					$save2[]= $this->db->query("INSERT INTO inventory2 set ".$data);
				}
			}
	...
	function delete_receiving2(){
		extract($_POST);
		$del1 = $this->db->query("DELETE FROM receiving_list2 where id = $id ");
		$del2 = $this->db->query("DELETE FROM inventory2 where type = 1 and form_id = $id ");
		if($del1 && $del2)
			return 1;
	}
	...
	function chk_prod_availability2(){
		extract($_POST);
		$price = $this->db->query("SELECT * FROM product_list2 where id = ".$id)->fetch_assoc()['price'];
		$inn = $this->db->query("SELECT sum(qty) as inn FROM inventory2 where type = 1 and product_id = ".$id);
		$inn = $inn && $inn->num_rows > 0 ? $inn->fetch_array()['inn'] : 0;
		$out = $this->db->query("SELECT sum(qty) as `out` FROM inventory2 where type = 2 and product_id = ".$id);
		$out = $out && $out->num_rows > 0 ? $out->fetch_array()['out'] : 0;
		$ex = $this->db->query("SELECT sum(qty) as ex FROM expired_product2 where product_id = ".$id);
		$ex = $ex && $ex->num_rows > 0 ? $ex->fetch_array()['ex'] : 0;
		$available = $inn - $out - $ex;
		return json_encode(array('available'=>$available,'price'=>$price));
	...
	function save_sales2(){
		...
		if(empty($id)){
			...
			while($i == 1){
				$chk = $this->db->query("SELECT * FROM sales_list2 where ref_no ='$ref_no'")->num_rows;
				if($chk > 0){
					$ref_no = mt_rand(1,99999999);
					$ref_no = sprintf("%'.08d\n", $ref_no);
				}else{
			...
			$data .= ", ref_no = '$ref_no' ";
			$save = $this->db->query("INSERT INTO sales_list2 set ".$data);
			$id =$this->db->insert_id;
			foreach($product_id as $k => $v){
				$data = " form_id = '$id' ";
				$data .= ", product_id = '$product_id[$k]' ";
			...
			$save = $this->db->query("UPDATE sales_list2 set ".$data." where id=".$id);
			$ids = implode(",",$inv_id);
			$this->db->query("DELETE FROM inventory2 where type = 1 and form_id ='$id' and id NOT IN (".$ids.") ");
			foreach($product_id as $k => $v){
				$data = " form_id = '$id' ";
	...
	function save_expired2(){
		...
		foreach ($product_id as $key => $value) {
			...
			$save[] = $this->db->query("INSERT INTO expired_product2 set $data ");
		}
		if(isset($save))
			return 1;
	}
	...
	function delete_expired2(){
		extract($_POST);
		$delete = $this->db->query("DELETE FROM expired_product2 where id = $id ");
		if($delete)
			return 1;
	}
}