	...
	<div class="col-lg-12">
		...
		<div class="row">
			...
			<div class="col-md-6" style="float:left;">
				<div class="card">
					<div class="card-header">
						<b>Receiving List (Wholesale)</b>
							<button class="col-md-2 float-right btn btn-primary btn-sm" id="new_receiving2"><i class="fa fa-plus"></i> New Receiving</button>
					</div>
					<div class="card-body">
						<table id="table2" class="table table-bordered">
							...
							<?php 
								...
								endwhile;
								$i = 1;
								$receiving = $conn->query("SELECT * FROM receiving_list2 r order by date(date_added) desc");
								while($row=$receiving->fetch_assoc()):
							?>
								<tr>
									<td class="text-center"><?php echo $i++ ?></td>
									...
									<td class=""><?php echo isset($sup_arr[$row['supplier_id']])? $sup_arr[$row['supplier_id']] :'N/A' ?></td>
									<td class="text-center">
										<a class="btn btn-sm btn-primary" href="index.php?page=manage_receiving2&id=<?php echo $row['id'] ?>">Edit</a>
										<a class="btn btn-sm btn-danger delete_receiving2" href="javascript:void(0)" data-id="<?php echo $row['id'] ?>">Delete</a>
									</td>
								</tr>
							<?php endwhile; ?>
							</tbody>
...
<script>
	...
	$('#table2').dataTable()
	$('#new_receiving2').click(function(){
		location.href = "index.php?page=manage_receiving2"
	})
	$('.delete_receiving2').click(function(){
		_conf("Are you sure to delete this data?","delete_receiving2",[$(this).attr('data-id')])
	})
	function delete_receiving2($id){
		start_load()
		$.ajax({
			url:'ajax.php?action=delete_receiving2',
			method:'POST',
			data:{id:$id},
			success:function(resp){
				if(resp==1){
