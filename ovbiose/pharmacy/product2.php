<?php include('db_connect.php');
	...
	$i = 1;
	while($i == 1){
		$chk = $conn->query("SELECT * FROM product_list2 where sku ='$sku'")->num_rows;
		if($chk > 0){
			$sku = mt_rand(1,99999999);
			$sku = sprintf("%'08d\n", $sku);
		}else{
					...
					<div class="card-body">
						<table class="table table-bordered table-hover">
							...
							<tbody>
								<?php 
								$i = 1;
								$prod = $conn->query("SELECT * FROM product_list2 order by id asc");
...
<script>
	...
	$('#manage-product').submit(function(e){
		...
		start_load()
		$.ajax({
			url:'ajax.php?action=save_product2',
	...
	function delete_product($id){
		start_load()
		$.ajax({
			url:'ajax.php?action=delete_product2',
			method:'POST',
			data:{id:$id},
			success:function(resp){
				if(resp==1){
