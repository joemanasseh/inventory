					...
					<div class="card-body">
						<table class="table table-bordered">
							...
							<?php 
								...
								$i = 1;
								$sales = $conn->query("SELECT * FROM sales_list2  order by date(date_updated) desc");
								while($row=$sales->fetch_assoc()):
							?>
								<tr>
									<td class="text-center"><?php echo $i++ ?></td>
									...
									<td class=""><?php echo isset($cus_arr[$row['customer_id']])? $cus_arr[$row['customer_id']] :'N/A' ?></td>
									<td class="text-center">
										<a class="btn btn-sm btn-primary" href="index.php?page=pos2&id=<?php echo $row['id'] ?>">Edit</a>
