...
if(isset($_GET['id'])){
	$qry = $conn->query("SELECT * FROM receiving_list2 where id=".$_GET['id'])->fetch_array();
	foreach($qry as $k => $val){
		$$k = $val;
	}
	$inv = $conn->query("SELECT * FROM inventory2 where type=1 and form_id=".$_GET['id']);
	...
	<div class="col-lg-12">
		<div class="card">
			<div class="card-header">
				<h4>Manage Receiving (Wholesale)</h4>
			</div>
			<div class="card-body">
...
<script>
	...
	$('#manage-receiving').submit(function(e){
		...
		$.ajax({
			url:'ajax.php?action=save_receiving2',
		    method: 'POST',
		    data: $(this).serialize(),
