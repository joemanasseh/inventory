...
if(isset($_GET['id'])){
	$qry = $conn->query("SELECT * FROM sales_list2 where id=".$_GET['id'])->fetch_array();
	foreach($qry as $k => $val){
		$$k = $val;
	}
	$inv = $conn->query("SELECT * FROM inventory2 where type=2 and form_id=".$_GET['id']);
...
<script>
	...
	$('#manage-sales').submit(function(e){
		...
		$.ajax({
			url:'ajax.php?action=save_sales2',
		    method: 'POST',
		    data: $(this).serialize(),
			success:function(resp){
				if(resp > 0){
					...
					var nw = window.open("print_sales2.php?id="+resp,"_blank","height=700,width=900")
						nw.print()
						setTimeout(function(){
							nw.close()
							location.reload()
