...
if(isset($_GET['iid'])){
	$conn->query("UPDATE inventory2 set expired_confirmed = 1 where id = ".$_GET['iid']);
}
						...
						<div class="row mb-3">
								<div class="col-md-4">
									...
									$cat = $conn->query("SELECT * FROM category_list order by name asc");
										...
										endwhile;
									$product = $conn->query("SELECT * FROM product_list2  order by name asc");
						...
						<div class="row">
							<table class="table table-bordered" id="list">
								...
								<tbody>
									<?php 
									if(isset($_GET['iid'])):
										$inv = $conn->query("SELECT i.*,p.name,p.measurement,p.sku,p.description FROM inventory2 i inner join product_list2 p on p.id = i.product_id where i.id =".$_GET['iid']);
...
<script>
	...
	$('#search_prod').click(function(){
		uni_modal("Find Available Product.","find_product2.php","large")
	})
	...
	$('#manage-expired').submit(function(e){
		...
		$.ajax({
			url:'ajax.php?action=save_expired2',
		    method: 'POST',
		    data: $(this).serialize(),
